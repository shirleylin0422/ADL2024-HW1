...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of data points to generate loss curve",
    )
    parser.add_argument(
        "--em_data_point_num",
        type=int,
        default=5,
        help="Number of data points to generate EM metric curve",
    )
    parser.add_argument(
        "--report_to",
        type=str,
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                # if accelerator.is_local_main_process:
                    # print(f"Epoch: {epoch}, Step: {completed_steps}, Loss: {loss.item()}")
            ...
            if accelerator.sync_gradients:
                ...
                if args.with_tracking and completed_steps%(args.max_train_steps//args.loss_data_point_num) == 0:
                    avg_loss = total_loss.item() / (step+1)

                    avg_loss = total_loss.item() / (step+1)
                    train_losses_avg.append(avg_loss)
                    train_losses.append(loss.item())
                    train_steps.append(completed_steps+1) 
                    # print("loss: ", loss.item())
                    # print("total_loss: ", total_loss.item())
                    # print("avg_loss: ", avg_loss)
                    # print("train_losses: ", train_losses)
                    # print("train_losses_avg: ", train_losses_avg)
                    ...
                    # plt.plot(train_steps, train_losses_avg, label="Training Avg Loss")
                    # plt.legend()
                    # plt.savefig('current_loss_curve.png')
                    
                    
                ###################################################################################################
                # Plot EM
                if args.with_tracking and completed_steps%(args.max_train_steps//args.em_data_point_num) == 0:
                    # print("------------------------eval------------------------")
                    all_start_logits = []
                    all_end_logits = []
                    model = model.to(to_cuda)
                    ...
                    for step_,val_batch in enumerate(eval_dataloader):
                        with torch.no_grad():
                    ...
                    eval_metric_ = evaluate.load("squad_v2" if args.version_2_with_negative else "squad")
                    prediction = post_processing_function(eval_examples, eval_dataset, outputs_numpy)
                    eval_exact_match  = eval_metric_.compute(predictions=prediction.predictions, references=prediction.label_ids)["exact_match"]
                    eval_exact_matches.append(eval_exact_match)
                    eval_steps.append(completed_steps+1) 
                    # print("eval_exact_match: ",eval_exact_match)
                    # print("eval_exact_matches[]: ",eval_exact_matches)
                    
                    #  # Exact Match Curve
                    # plt.figure() 
                    # plt.plot(eval_steps, eval_exact_matches, label="Validation Exact Match")
                    # plt.xlabel("Steps")
                    # plt.ylabel("Exact Match (%)")
                    # plt.title("Validation Exact Match Curve")
                    # if completed_steps ==0:
                    #     plt.legend()
                    # plt.savefig('current_EM_curve.png')
